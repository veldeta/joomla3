...
class VideogalleryliteControllerGeneral extends JControllerForm
{
   function save($key = null, $urlVar = null) {
     ...
     function  cancel($key = NULL){
        ...
        function apply1(){
             $model = $this->getModel('General');
             $item=$model->save('');
             $this->setRedirect(JRoute::_('index.php?option=com_videogallerylite&view=general', false), JText::_('COM_VIDEOGALLERYLITE_SETTINGS'));
        }
        function save1(){
          $model = $this->getModel('General');
             $item=$model->save('');
